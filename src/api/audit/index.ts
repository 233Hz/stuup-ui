...
export const pageGrowAuditRecord = async (params: Page): Promise<ApiResult<PageResult<GrowAuditRecordVO[]>>> => {
...
export const refuseGrowItem = async (id: number, reason: string): Promise<ApiResult<boolean>> => {
  return get('/audGrow/refuse/' + id, { reason });
};

export const returnGrowItem = async (id: number, reason: string): Promise<ApiResult<boolean>> => {
