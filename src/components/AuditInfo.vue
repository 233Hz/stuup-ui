      ...
      <div class="info-content">
        <el-scrollbar>
          <el-descriptions direction="vertical" :column="1" border>
            <el-descriptions-item label="一级项目">{{ applyRecord?.firstLevelName }}</el-descriptions-item>
            <el-descriptions-item label="二级项目">{{ applyRecord?.secondLevelName }}</el-descriptions-item>
            <el-descriptions-item label="三级项目">{{ applyRecord?.thirdLevelName }}</el-descriptions-item>
            <el-descriptions-item label="成长项目">{{ applyRecord?.growName }}</el-descriptions-item>
            <el-descriptions-item label="申请原因">{{ applyRecord?.reason }}</el-descriptions-item>
            <el-descriptions-item label="证明附件">
              <el-table :data="fileData" border stripe v-loading="loading" empty-text="未上传附件" style="width: 100%">
                <el-table-column prop="originalName" label="文件名" show-overflow-tooltip align="center" />
                <el-table-column prop="createTime" label="上传时间" show-overflow-tooltip align="center" />
                <el-table-column label="操作" width="120" align="center">
                  <template #default="{ row }">
                    <el-button @click="downFile(row.storageName, row.originalName)">下载文件</el-button>
                  </template>
                </el-table-column>
              </el-table>
            ...
            <el-timeline-item v-for="item in auditLog" :key="item.id" :timestamp="item.createTime" placement="top">
              <el-card>
                <h3>
                  ...
                  <el-tag v-if="item.state === AUDIT_LOG_STATUS.PASS" type="success">
                    {{ AUDIT_LOG_STATUS.getKey('PASS') }}
                  </el-tag>
                  <el-tag v-if="item.state === AUDIT_LOG_STATUS.REFUSE" type="danger">
                    {{ AUDIT_LOG_STATUS.getKey('REFUSE') }}
                  </el-tag>
                  <el-tag v-if="item.state === AUDIT_LOG_STATUS.RETURN" type="warning">
